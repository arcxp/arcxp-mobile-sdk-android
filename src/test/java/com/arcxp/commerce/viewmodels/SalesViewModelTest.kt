...
class SalesViewModelTest {
    ...
    @Test
    fun `getEntitlements V2- successful response with callback`() = runTest {
        val response = arcXPEntitlements


        coEvery {
            salesRepository.getEntitlementsV2()
        } returns Success(response)

        testObject.getEntitlementsV2(listener)

        coVerify {
            salesRepository.getEntitlementsV2()
            listener.onGetEntitlementsV2Success(arcXPEntitlements)
        }

    }

    @Test
    fun `getEntitlements V2 - successful response without callback`() = runTest {
        val response = arcXPEntitlements
        ...
        testObject.getEntitlementsV2(null)

        coVerify {
            salesRepository.getEntitlementsV2()
        }
        assertEquals(response, testObject.entitlementsV2Response.value)

    }

    @Test
    fun `getEntitlements V2 - failed response with callback`() = runTest {
        val response = Failure(ArcXPException("Failed"))
        ...
        coVerify {
            salesRepository.getEntitlementsV2()
            listener.onGetEntitlementsV2Failure(response.failure)
        }

    }

    @Test
    fun `getEntitlements V2 - failed response without callback`() = runTest {
        val response = ArcXPException("Failed")
