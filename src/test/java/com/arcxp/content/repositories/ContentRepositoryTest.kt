...
import com.arcxp.content.sdk.db.CacheManager
import com.arcxp.content.sdk.db.CollectionItem
import com.arcxp.content.sdk.db.JsonItem
import com.arcxp.content.sdk.db.SectionHeaderItem
...
class ContentRepositoryTest {
    ...
    fun `getContentSuspend failure from api`() = runBlocking {
        ...
        val expected = Failure(expectedError)
        coEvery {
            cacheManager.getJsonById(id = id)
        } returns null
        coEvery {
    ...
    fun `getCollectionSuspend returns db result (shouldIgnore False, stale False)`() = runBlocking {
        ...
        coEvery {
            ...
            CollectionItem(
                ...
                indexValue = 0,
                collectionResponse = collectionJson0,
                expiresAt = expirationDate.time
            )
        )
    ...
    fun `getCollectionSuspend returns api result (shouldIgnore true)`() = runBlocking {
        ...
        list.forEachIndexed { index, arcXPCollection -> map[index] = arcXPCollection }
        val expected = Success(map)
        coEvery {
            contentApiManager.getCollection(
                id = id,
    ...
    fun `getCollectionSuspend with empty result from db, inserts into db`() = runBlocking {
        ...
        assertEquals(expected, actual)
        val dbInsertionSlot = mutableListOf<CollectionItem>()
        coVerify(exactly = 3) { cacheManager.insertCollectionItem(capture(dbInsertionSlot)) }
        assertEquals(
            collectionList[0],
            fromJson(dbInsertionSlot[0].collectionResponse, ArcXPCollection::class.java)
        )
        assertEquals(
            collectionList[1],
            fromJson(dbInsertionSlot[1].collectionResponse, ArcXPCollection::class.java)
        )
        assertEquals(
            collectionList[2],
            fromJson(dbInsertionSlot[2].collectionResponse, ArcXPCollection::class.java)
        )
        assertEquals(id, dbInsertionSlot[0].contentAlias)
        assertEquals(id, dbInsertionSlot[1].contentAlias)
        assertEquals(id, dbInsertionSlot[2].contentAlias)
        assertEquals(0, dbInsertionSlot[0].indexValue)
        assertEquals(1, dbInsertionSlot[1].indexValue)
        assertEquals(2, dbInsertionSlot[2].indexValue)
    }
        ...
        runBlocking {
            ...
            coEvery {
                ...
                CollectionItem(
                    contentAlias = "contentAlias",
                    indexValue = 1,
                    collectionResponse = collectionJson1,
                    createdAt = cacheDate.time,
                    expiresAt = cacheDate.time
                ),
                CollectionItem(
                    contentAlias = "contentAlias",
                    indexValue = 2,
                    collectionResponse = collectionJson2,
                    createdAt = cacheDate.time,
                    expiresAt = cacheDate.time
                )
            )
            ...
            coVerify(exactly = 3) { cacheManager.insertCollectionItem(capture(dbInsertionSlot)) }
            assertEquals(
                fromJson(collectionJson0, ArcXPCollection::class.java),
                fromJson(dbInsertionSlot[0].collectionResponse, ArcXPCollection::class.java)
            )
            assertEquals(
                fromJson(collectionJson1, ArcXPCollection::class.java),
                fromJson(dbInsertionSlot[1].collectionResponse, ArcXPCollection::class.java)
            )
            assertEquals(
                fromJson(collectionJson2, ArcXPCollection::class.java),
                fromJson(dbInsertionSlot[2].collectionResponse, ArcXPCollection::class.java)
            )
            assertEquals("contentAlias", dbInsertionSlot[0].contentAlias)
            assertEquals("contentAlias", dbInsertionSlot[1].contentAlias)
            assertEquals("contentAlias", dbInsertionSlot[2].contentAlias)
            assertEquals(0, dbInsertionSlot[0].indexValue)
            assertEquals(1, dbInsertionSlot[1].indexValue)
            assertEquals(2, dbInsertionSlot[2].indexValue)
        }
    ...
    fun `getCollectionJsonSuspend on success`() = runBlocking {
        ...
        val expected = Success(success = expectedJson)
        coEvery {
            contentApiManager.getCollection(id = id, from = 837, size = 983)
        } returns expectedResponse
